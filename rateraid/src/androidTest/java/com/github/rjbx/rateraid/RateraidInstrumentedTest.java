...
public class RateraidInstrumentedTest {
    ...
    private long mCount;
    private Rateraid.Arrays mRateraid;
    ...
    public final void testAddButtonSetClickListenerCallback() {
        ...
        for (float magnitude = 0.01f; magnitude < 0.1f; magnitude += 0.01f) {
            mRateraid = Rateraid.with(percentages, magnitude, PRECISION, null);
            for (int index = 0; index < 4; index++) {

                mRateraid.addShifters(mIncrementButton, mDecrementButton, index);
                mRateraid.instance();
                float sum;
    ...
    public final void testSetClickListenerUserDefinedCallback() {
        double[] percentages = { .25f, .25f, .25f, .25f };
        for (float magnitude = 0.01f; magnitude < 0.1f; magnitude += 0.01f) {
            mRateraid = Rateraid.with(percentages, magnitude, PRECISION, clickedView -> mCount++);
            for (int index = 0; index < 4; index++) {
